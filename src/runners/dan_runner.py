...
class DANRunner:
    ...
    def run(self, agent='trainee', test_mode=False):
        self.reset()
        ...
        while True:
            ...
            if save_probs:
                actions, probs = self.mac.select_actions(self.batch, t_ep=self.t, t_env=self.t_env, agent=agent,
                                                         bs=envs_not_terminated, test_mode=test_mode)
            else:
                actions = self.mac.select_actions(self.batch, t_ep=self.t, t_env=self.t_env, agent=agent,
                                                  bs=envs_not_terminated, test_mode=test_mode)
        ...
        if agent == 'tutor':
            log_prefix = "test_{}_".format(agent) if test_mode else ""
